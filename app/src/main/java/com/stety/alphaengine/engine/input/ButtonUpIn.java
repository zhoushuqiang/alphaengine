...
/**
 * Button, which responds on action up and only in trigger.
 * Responds on press:
...
public class ButtonUpIn extends ButtonDownOut {
    /**
     * Button, which responds on action up and only in trigger.
     * Responds on press:
     * 1) On pressed: if(pressed) and after you must (in if): pressed = false;
     ...
     */
    public ButtonUpIn(boolean pressedSwitch, boolean only) {
     ...
     * @param pressedSwitch Initial state for pressedSwitch (ON/OFF). (if you don't use pressedSwitch is it freely.)
     */
    public ButtonUpIn(boolean pressedSwitch) {
        super(pressedSwitch);
    }
    ...
    public void action_move(short x, short y) {
        if (x >= triggerUpperLeftCorner[0] && x <= triggerLowerRightCorner[0] && y >= triggerUpperLeftCorner[1] && y <= triggerLowerRightCorner[1]) {
            focus = true;
            if (Logs.Logs)
                Log.d(Logs.tagInput, "ButtonUpIn is focus.");
        } else {
            focus = false;
            if (Logs.Logs)
                Log.d(Logs.tagInput, "ButtonUpIn is unfocus.");
        }
    }
}